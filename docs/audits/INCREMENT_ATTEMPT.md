...
This folder claimed auxiliary identities only. `CORE_AUXILIARY_IDENTITIES.md` (then the root README) explicitly leaves
the density-increment mechanism unresolved. Its finite examples are not a
proof of `r_4(N) = o(N/log N)`.

Read before this attempt: `CORE_AUXILIARY_IDENTITIES.md`, `verification/scripts/verify_core.py`, `verification/results/verification.json`,
...
Captured stdout, matching the existing `verification/results/verification.json`:
...
```bash
python3 verification/scripts/verify_increment.py > verification/results/increment_verification.json
```
...
[`verification/results/increment_verification.json`](../../verification/results/increment_verification.json).
...
`python3 verification/scripts/verify_increment.py` and checking the
