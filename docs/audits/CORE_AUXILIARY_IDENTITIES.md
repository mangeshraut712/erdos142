...
This file is the original root README from the 2026-09-06 offline pack,
kept here so the auxiliary identities are not lost after the repository
was reorganized. Current project status lives in the root `README.md`
and in `docs/PROGRESS.md`.
...
`python3 verification/scripts/verify_full_group_energy.py` for its exact finite checks.
...
`python3 verification/scripts/verify_endpoint_incidence.py` for its exact finite diagnostics.
...
`python3 verification/scripts/verify_ap_richness.py` for exact supporting checks.
...
`python3 verification/scripts/verify_energy_counterexample.py` for supporting exact finite checks.
...
`python3 verification/scripts/verify_localization.py` for the new exact finite diagnostics.
...
normalization obstruction, and exponent ledger. Run `python3 verification/scripts/verify_phase2.py`
...
no sufficient arithmetic increment follows yet. Run `python3 verification/scripts/verify_signed.py`
